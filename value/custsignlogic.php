...
if($sid==0)
{
    if($name==''or $number==''or $password=='' or $cpassword=='')
    {
        ...
        if($f==0)
            {
                $query1="insert into signup (lvl,name,number, password) values('5','$name','$number','$password')";
                $result2=mysqli_query($con,$query1)or die(mysqli_error());
                date_default_timezone_set("asia/kolkata");
                $datetime = date('d/m/Y', time());
                $query2="insert into `customer` (name,number,joining_date) values('$name','$number','$datetime')";
